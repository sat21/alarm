...
class Alarm(tk.Frame):
    ...
    def add_widgets(self):
        ...
        self.active_button = tk.Checkbutton(self.active_frame, text="aktif",
                                            variable=self.active_var,
                                            relief=tk.RAISED)

        self.edit_frame = tk.Frame(self)
        self.edit_button = tk.Button(self.edit_frame, text="düzenle", command=self.edit_alarm)
        self.delete_button = tk.Button(self.edit_frame, text="sil", command=self.delete)
    ...
    def edit_alarm(self):
        ...
        self.set_pm = tk.Radiobutton(self.am_pm_frame, text="PM", variable=self.am_pm,
                                     value=1)
        self.active_button = tk.Checkbutton(self.edit_frame, text="ayarla", variable=self.new_active)
        self.save_button = tk.Button(self.edit_window, text="kaydet", command=self.confirm_edit)
        self.cancel_button = tk.Button(self.edit_window, text="iptal etmek", command=self.edit_window.destroy)
    ...
    def confirm_edit(self):
        self.confirm = messagebox.askyesno(message="Bu işlemi tamamlamak istediğinizden emin misiniz?", icon="soru", title="Eylemi Onayla",
                            default="yes")
        if self.confirm == "yes":
            ...
            if bool(active) is True:
                ...
                print("Etkin değişkende bir hata oluştu:", active)
                return
            if __name__ != "__main__":
...
if __name__ == "__main__":
    ...
    alarm = Alarm(root, 1, "12:00 PM", "PztSalÇarPerCumCmtPaz", None, "", True)
