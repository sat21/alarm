...
class Alarms(tk.Frame):
    def __init__(self, parent):
        ...
        self.add_alarm_button = tk.Button(self, text="Alarm ekle", command=self.add_alarm)
    ...
    def add_alarms(self):
        ...
        for row in range(len(self.alarms_data)):
            self.alarm_frames.append(Alarm(self.alarms, self.alarms_data[row][0],
                                               self.alarms_data[row][1], "PazPztSalÇarPerCumCmts",
    ...
    def add_alarm(self):
        ...
        self.set_pm = tk.Radiobutton(self.am_pm_frame, text="PM", variable=self.am_pm,
                                     value=1)
        self.active_button = tk.Checkbutton(self.add_frame, text="aktif:", variable=self.new_active)
        self.save_button = tk.Button(self.add_window, text="kaydet", command=self.confirm_add_alarm)
        self.cancel_button = tk.Button(self.add_window, text="iptal", command=self.add_window.destroy)
    ...
    def confirm_add_alarm(self):
        self.confirm = messagebox.askyesno(message="Bu işlemi tamamlamak istediğinizden emin misiniz?", icon="question", title="eylemi onayla",
                            default="yes")
        if self.confirm is True:
            ...
            if am_pm == 0:
                ...
                print("am_pm değerinde bir hata oluştu:", am_pm)
                raise ValueError
            ...
            if active == 0:
                ...
                print("Etkin değerde bir hata oluştu:", active)
                raise ValueError
