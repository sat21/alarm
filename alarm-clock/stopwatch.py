...
class Stopwatch(tk.Frame):
    def __init__(self, parent):
        ...
        self.active_button = tk.Button(self.button_frame, text="başla", command=self.start)
        self.restart_button = tk.Button(self.button_frame, text="yeniden",
                                        command=self.restart)
        self.save_button = tk.Button(self.button_frame, text="kaydet", command=self.save)
        self.clear_button = tk.Button(self.button_frame, text="temizle",
                                      command=self.clear)
        self.saved_canvas = tk.Canvas(self, width=150)
        self.saved_frame = tk.LabelFrame(self.saved_canvas, text="kayıtlar")
    ...
    def start(self):
        self.active = True
        self.active_button.config(text="dur", command=self.pause)

    def pause(self):
        self.active = False
        self.active_button.config(text="başla", command=self.start)
